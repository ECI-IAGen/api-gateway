...
public class ExcelImportService {
    ...
    public ExcelImportResponseDTO importExcel(MultipartFile file) {
        ...
        try {
            ...
            try (InputStream inputStream = file.getInputStream();
                ...
                // PASO 2: Crear el equipo/grupo
                log.info("=== PASO 2: Creando equipo/grupo ===");
                Team courseTeam = createOrUpdateTeam(classInfo.getCourseName(), classInfo.getCourseCode());
                if (courseTeam != null) {
                    stats.setTeamsCreated(stats.getTeamsCreated() + 1);
                    log.info("Equipo creado: {}", courseTeam.getName());
                }

                // PASO 3: Crear o actualizar profesor
                log.info("=== PASO 3: Creando profesor ===");
                ...
                processStudents(sheet, courseTeam, stats, errors);

                stats.setTotalProcessed(stats.getUsersCreated() + stats.getUsersUpdated());

                String message = String.format(
                        "Importación completada. Roles: %d creados. Usuarios: %d creados, %d actualizados. Equipos: %d creados",
                        stats.getRolesCreated(), stats.getUsersCreated(), stats.getUsersUpdated(),
                        stats.getTeamsCreated());
    ...
    private void processStudents(Sheet sheet, Team courseTeam, ExcelImportResponseDTO.ImportStats stats,
            List<String> errors) {
        ...
        for (int i = startRow; i <= sheet.getLastRowNum(); i++) {
            ...
            try {
                ...
                if (student != null) {
                    ...
                    // Agregar estudiante al equipo si no está ya
                    if (courseTeam != null) {
                        addUserToTeam(student, courseTeam);
                        log.info("Estudiante {} agregado al equipo {}", studentName, courseTeam.getName());
                    }
    ...
    private Team createOrUpdateTeam(String name, String code) {
        try {
            ...
            log.error("Error creando/actualizando equipo: {}", name, e);
            return null;
        }
    }
